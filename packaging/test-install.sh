...
echo "=== logodev-mcp .deb install test ==="

# Find the .deb
DEB=$(find dist/ -name 'logodev-mcp*.deb' -print -quit 2>/dev/null || true)
if [ -z "$DEB" ]; then
...
if getent passwd logodev-mcp >/dev/null 2>&1; then
    pass "System user 'logodev-mcp' exists"
else
    fail "System user 'logodev-mcp' not found"
fi

# 2. System group exists
if getent group logodev-mcp >/dev/null 2>&1; then
    pass "System group 'logodev-mcp' exists"
else
    fail "System group 'logodev-mcp' not found"
fi

# 3. State directory exists with correct ownership and permissions
if [ -d /var/lib/logodev-mcp ]; then
    OWNER=$(stat -c '%U:%G' /var/lib/logodev-mcp)
    MODE=$(stat -c '%a' /var/lib/logodev-mcp)
    if [ "$OWNER" = "logodev-mcp:logodev-mcp" ] && [ "$MODE" = "750" ]; then
        pass "State directory owned by logodev-mcp with mode 0750"
    elif [ "$OWNER" != "logodev-mcp:logodev-mcp" ]; then
        fail "State directory owned by $OWNER (expected logodev-mcp:logodev-mcp)"
    else
        fail "State directory mode $MODE (expected 750)"
    fi
else
    fail "/var/lib/logodev-mcp does not exist"
fi

# 4. Install directory exists
if [ -d /opt/logodev-mcp ]; then
    pass "Install directory /opt/logodev-mcp exists"
else
    fail "Install directory /opt/logodev-mcp does not exist"
fi

# 4b. Venv installed by postinstall
if [ -x /opt/logodev-mcp/venv/bin/python3 ]; then
    pass "venv installed at /opt/logodev-mcp/venv"
else
    fail "venv not found at /opt/logodev-mcp/venv (postinstall may have failed)"
fi

# 5. Unit file exists
if [ -f /usr/lib/systemd/system/logodev-mcp.service ]; then
...
if command -v systemd-analyze >/dev/null 2>&1; then
    if output=$(systemd-analyze verify /usr/lib/systemd/system/logodev-mcp.service 2>&1); then
