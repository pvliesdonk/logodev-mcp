...
This standard governs **first-party code only** — `src/logodev_mcp/` and
...
Domain configuration composes `fastmcp_pvl_core.ServerConfig` inside your domain config class (see `src/logodev_mcp/config.py`).  Add domain fields between the `CONFIG-FIELDS-START` / `CONFIG-FIELDS-END` sentinels and populate them in `from_env` between the `CONFIG-FROM-ENV-START` / `CONFIG-FROM-ENV-END` sentinels.  Never inherit from `ServerConfig`; always compose.

Env var prefix is `LOGODEV_MCP_` — all env reads go through `fastmcp_pvl_core.env(_ENV_PREFIX, "SUFFIX", default)` so naming stays consistent.
...
- **Plan-tier gating in the lifespan.** logo.dev exposes no plan endpoint, so `server_lifespan` (`_server_deps.py`) calls `Service.probe_entitlements()` after startup — probing `describe`/`brand` on `logo.dev` — and `mcp.disable(names=...)` (FastMCP 3.x uses bare tool names, not `tool:`-prefixed keys) for any `401`/`403`. Ambiguous results (timeout, `404`, `429`, `5xx`) fail open. The verdict is cached for `ENTITLEMENT_TTL_SECONDS` (7 days) under `config.state_dir`, keyed by a sha256 fingerprint of the secret key so rotation re-probes. Gated off by `config.detect_plan` (`LOGODEV_MCP_DETECT_PLAN=false`); the test suite defaults it off (see `conftest.py`).
